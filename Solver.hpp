...
// This is 4th-order accurate and requires 5 stages.
// `ark` stores the intermediate a_i coefficients used in stage-state updates;
// `brk` stores the b_i weights used in accumulation updates.
...
// This is 5th-order accurate and requires 9 stages.
// `ark` stores the intermediate a_i coefficients used in stage-state updates;
// `brk` stores the b_i weights used in accumulation updates.
// `ark` has RK_STAGES-1 entries (a_i), `brk` has RK_STAGES entries (b_i).
const float ark[] = {
